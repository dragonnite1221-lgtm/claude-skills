...
description: "Audit project dependencies for security vulnerabilities (CVE matching with CVSS scores), license compliance/conflicts, and safe upgrade paths across 8+ ecosystems — npm (package.json/lock, yarn.lock), Python (requirements.txt/poetry.lock), Go (go.mod), Rust (Cargo.lock), Ruby, Maven, PHP, .NET. Use when the user wants to scan dependencies for CVEs, check licenses (GPL/MIT/Apache compatibility), find outdated packages, plan version upgrades with breaking-change risk, or gate dependency security in CI — e.g. 'audit my dependencies', 'are any of my packages vulnerable', 'check license compliance', or 'plan a safe upgrade'."
---
...
project). See [license_compatibility_matrix.md](references/license_compatibility_matrix.md).
...
- [vulnerability_assessment_guide.md](references/vulnerability_assessment_guide.md) — CVE/CVSS scoring, triage, false-positive handling
- [license_compatibility_matrix.md](references/license_compatibility_matrix.md) — license classes and conflict rules
- [dependency_management_best_practices.md](references/dependency_management_best_practices.md) — scan cadence, upgrade hygiene, supply-chain risk
