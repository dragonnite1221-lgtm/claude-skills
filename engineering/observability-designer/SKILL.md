...
description: "Design production observability — SLI/SLO frameworks, dashboards, and alert tuning — from a service-description JSON. slo_designer.py turns service type/criticality/user-facing into SLIs, SLO targets, error budgets, and multi-window burn-rate alerts. dashboard_generator.py emits a Grafana-compatible dashboard with golden-signals/RED/USE panels and role-based views (sre/developer/executive/ops). alert_optimizer.py analyzes an existing alert config for noise, duplicates, coverage gaps, and bad thresholds and emits an optimized config (JSON or HTML report). Use when the user wants to define SLOs/error budgets, build a Grafana dashboard, reduce alert fatigue, or asks 'set up monitoring for this service', 'what SLOs should this API have', or 'my alerts are too noisy'."
license: MIT
metadata:
  tier: POWERFUL
...
raw thresholds. See [slo_cookbook.md](references/slo_cookbook.md).
...
and SLO reference lines. See [dashboard_best_practices.md](references/dashboard_best_practices.md).
...
during known outages. See [alert_design_patterns.md](references/alert_design_patterns.md).

## References

| File | Contents |
|------|----------|
| [slo_cookbook.md](references/slo_cookbook.md) | SLI selection, SLO targets, error budgets, burn-rate alerting |
| [dashboard_best_practices.md](references/dashboard_best_practices.md) | Golden signals, RED/USE, layout, visualization choices |
| [alert_design_patterns.md](references/alert_design_patterns.md) | Severity, actionability, fatigue prevention, threshold selection |
