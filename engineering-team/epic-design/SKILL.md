...
description: >
  ...
  parallax depth, text animations, and premium scroll effects with CSS + GSAP — no WebGL.
  Inspects every image asset (inspect-assets.py), judges whether each background needs
  removing, assigns a 0–5 depth layer, then generates a self-contained HTML page with
  GPU-safe animations and reduced-motion fallbacks (validated by validate-layers.js).
  Covers 45+ techniques across 8 categories: parallax, pinned/scrub timelines, clip-path
  reveals, floating products between sections, card stacks, iris/curtain transitions,
  word-by-word text lighting, and split-converge typography. Use for landing pages, product
  sites, and hero sections, or on phrases like "make it feel alive", "Apple-style animation",
  "sections that overlap", "product rises between sections", "immersive", or "scrollytelling".
license: MIT
metadata:
  version: 1.0.0
  author: Abbas Mir
...
[asset-pipeline.md](references/asset-pipeline.md).
...
[depth-system.md](references/depth-system.md).
...
Details: [performance.md](references/performance.md), [accessibility.md](references/accessibility.md).
...
| File | What's inside |
|------|--------------|
| [asset-pipeline.md](references/asset-pipeline.md) | Asset inspection, bg-judgment rules, notification format, CSS knockout, resize targets |
| [depth-system.md](references/depth-system.md) | 6-layer depth model, CSS/JS, blur/scale formulas |
| [motion-system.md](references/motion-system.md) | 9 scroll architecture patterns with full GSAP code |
| [text-animations.md](references/text-animations.md) | 13 text techniques with implementation code |
| [directional-reveals.md](references/directional-reveals.md) | 8 clip-path "born from top/sides" techniques |
| [inter-section-effects.md](references/inter-section-effects.md) | Floating product, GSAP Flip, cross-section travel |
| [performance.md](references/performance.md) | GPU rules, will-change, IntersectionObserver patterns |
| [accessibility.md](references/accessibility.md) | WCAG 2.1 AA, prefers-reduced-motion, ARIA |
| [examples.md](references/examples.md) | 5 complete real-world implementations |
