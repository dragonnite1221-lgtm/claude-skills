...
description: "Product management toolkit covering RICE prioritization, customer-interview synthesis, PRD authoring, discovery frameworks, and go-to-market. Runs rice_prioritizer.py to score features by (Reach x Impact x Confidence) / Effort with portfolio balance and quarterly capacity planning, and customer_interview_analyzer.py to extract pain points, feature requests, jobs-to-be-done, sentiment, themes, and quotes from transcripts. Ships PRD templates (standard, one-page, brief, agile epic) and framework references (RICE, MoSCoW, Kano, JTBD). Use when the user says 'prioritize these features', 'run RICE', 'build a roadmap from this backlog', 'analyze this customer interview', 'synthesize user research', 'write a PRD', or 'plan sprint capacity'."
---
